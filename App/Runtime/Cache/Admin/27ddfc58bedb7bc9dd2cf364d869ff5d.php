                ...
                <div class="content-box">				
                    ...
                    <div class="content-box-content">			
                        <div class="tab-content default-tab" id="tab1"> <!-- This is the target div. id must match the href of this div's tab -->
                          <form action="<?php echo U('editAccessAction');?>" name="editForm" method="post" id="editForm" enctype="multipart/form-data" >
                          
                             <input type="hidden" name="id" value="<?php echo ($title["id"]); ?>" >
                            <p>
                              <label>Title:</label>                                
                             <input class="text-input small-input" type="text" name="title"  value="<?php echo ($title["title"]); ?>" />               
                            </p>  
                           
                            <p>
                            <div id="accordion">
                                <?php if(is_array($leve1)): $i = 0; $__LIST__ = $leve1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><h3 style="padding: 5px 0px 5px 30px; border:1px solid #999999"><?php echo ($key); ?></h3>
                                    <div>
                                        <?php if(is_array($vo)): foreach($vo as $key=>$v): ?><label><?php echo ($rule); ?><input name="rule[]" type="checkbox" value="<?php echo ($v); ?>"   <?php if(in_array(($v), is_array($arr)?$arr:explode(',',$arr))): ?>checked="checked"<?php endif; ?> >&nbsp;<span><?php echo ($key); ?></span></label><?php endforeach; endif; ?>
                            ...
                            <input class="button" type="button" onClick="window.location.href='<?php echo U('index');?>'"  value="Back" />      
                            </form>                  	
                        </div> <!-- End #tab1 --> 
                    </div>
                </div> 
