...
<li id="li_Menu_<?php echo ($i); ?>"> 
		...
		<div id="main-content"> <!-- Main Content Section with everything -->        
			...
			<div style="float:left; width:380px;">
			  <h2>News</h2></div>
			<div id="sreach">
            <form name="sreach" method="get" action="<?php echo U('index',array('cateid'=>$cateid));?>">
              <select name="news_status" onChange="document.getElementById('keyword').value='';document.sreach.submit(); return false;">
              <?php if(is_array($statusList)): foreach($statusList as $key=>$v): ?><option value="<?php echo ($v["value"]); ?>" <?php if($news_status == $v['value']): ?>selected=selected<?php endif; ?>><?php echo ($v["name"]); ?></option><?php endforeach; endif; ?>
              </select>
            ...
            <br>             
               <div class="content-box"><!-- Start Content Box -->                  
                  <div class="content-box-header">  
                      <span class="insert" style=" float:right; line-height:40px; font-size:14px; font-weight:bold;"> <a href="<?php echo U('addNews',array('cate_id'=>$cateid));?>">+ Insert</a>&nbsp; &nbsp; </span>
                    <div class="clear"></div>                      
                      ...
                      <!-- This is the target div. id must match the href of this div's tab -->	
                          ...
                          <table>
                            <thead>
                              <tr>								   
                                 <!-- <th>ID</th>-->
                                 <th width="50%">Title <?php echo createUpDownLink('news_title');?></th>
                                  <th   >Date <?php echo createUpDownLink('news_adddate');?></th>                               
                                  <th   style="text-align:center">Status</th>          	                                                                                                        
                                  <th  width="150" style="text-align:center"></th>									   						 
                              </tr>								
                            </thead> 
                            <tbody>                           
                               <?php if(is_array($newsList)): $i = 0; $__LIST__ = $newsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr <?php if(($mod) == "1"): ?>class="alt-row"<?php endif; ?>>	
                                    <td style="font-size:14px"><?php echo ($vo["news_title"]); ?></td>      
                                    <td ><?php echo (date('Y-m-d',strtotime($vo['news_adddate']))); ?></td>                                      
                                    <td style="text-align:center" id="<?php echo ($vo["news_id"]); ?>" class="status">
                                        <?php if($vo["news_status"] == 1): ?><a href="/yy/index.php/Admin/News/statsAction/ac/2/id/<?php echo ($vo["news_id"]); ?>/cateid/<?php echo ($cateid); ?>">On</a><?php else: ?>
                                        <a href="/yy/index.php/Admin/News/statsAction/ac/1/id/<?php echo ($vo["news_id"]); ?>/cateid/<?php echo ($cateid); ?>" class="stats">Off</a><?php endif; ?>                                
                                    </td>
                                   
                                    <td style="text-align:center;">   
                                      <a href="<?php echo U('editNews',array('news_id'=>$vo['news_id'],'cate_id'=>$cateid));?>" title="editNews">Edit</a> &nbsp; &nbsp;                              
                                      <a href="<?php echo U('deleteNews',array('news_id'=>$vo['news_id'],'cate_id'=>$cateid));?>" onClick="return confirm('Delete this news?')"  title="Delete">Delete</a> 
                                    </td>  
